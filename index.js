...
function title() {
    console.log(chalk.bold.green(figlet.textSync('abot Bot', {
        font: 'Standard',
...
const {
  ...
  if (abot.user && abot.user.id) abot.user.jid = jidNormalizedUser(abot.user.id)
...
app.use(express.static(path.join(__dirname, 'views')))
  server.listen(PORT, () => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.timedOut) { console.log("abotection TimedOut, Reconnecting..."); startabot(); }
            else abot.end(`Unknown DisconnectReason: ${reason}|${connection}`)
        }
        if (update.connection == "open" || update.receivedPendingNotifications == "true") {
				console.log('Connect, welcome owner!')
				console.log(`Connected to = ` + JSON.stringify(abot.user, null, 2))
			}
    })
    store.bind(abot.ev)
    
    abot.ev.on('call', async (celled) => {
    let botNumber = await abot.decodeJid(abot.user.id)
    ...
    let nomer = await abot.sendTextWithMentions(kopel.from, `*${abot.user.name}* tidak bisa menerima panggilan ${kopel.isVideo ? `video` : `suara`}. Maaf @${kopel.from.split('@')[0]} kamu akan diblokir. Silahkan hubungi Owner membuka blok !`)
    abot.sendContact(kopel.from, setting.ownerNumber.map( i => i.split("@")[0]), nomer)
    await sleep(8000)
    await abot.updateBlockStatus(kopel.from, "block")
    }
    }
    }
    })
    ...
    abot.ev.on('messages.upsert', async chatUpdate => {
        ...
        const m = smsg(abot, mek, store)
        require("./command/case")(abot, m, chatUpdate, mek, store, setting, isSetWelcome, getTextSetWelcome, set_welcome_db, set_left_db, isSetLeft, getTextSetLeft, _welcome, _left, antidelete, antionce)
        }
        } catch (err) {
            console.log(err)
        }
    ...
    abot.ev.on('group-participants.update', async (anu) => {
        ...
        welcome(iswel, isLeft, abot, anu)
    })
    abot.ev.on("message.delete", async (anu) => {
  const { aDelete } = require ("./lib/welcome");
		aDelete(setting, abot, anu)
	})
	abot.ev.on("viewOnceMessage", async (anu) => {
	  const { oneTime } = require ("./lib/welcome");
		oneTime(setting, abot, anu)
	})
	
	abot.ev.process(
		async (events) => {
			if (events['presence.update']) {
				await abot.sendPresenceUpdate('available')
			}
...
abot.sendButLoc = async (jid , text = '' , footer = '', lok, but = [], ments = [],  options = {}) =>{
      let bb = await abot.reSize(lok, 300, 150)
      const buttonMessage = {
 ...
 abot.sendMessage(jid, buttonMessage, options)
      }
      
abot.send5ButLoc = async (jid , text = '' , footer = '', lok, but = [], options = {}) =>{
      let bb = await abot.reSize(lok, 300, 150)
      abot.sendMessage(jid, { location: { jpegThumbnail: bb }, caption: text, footer: footer, templateButtons: but, ...options })
      }
abot.send5ButImg = async (jid , text = '' , footer = '', img, but = [], ments, options = {}) =>{
    abot.sendMessage(jid, { image: img, caption: text, footer: footer, viewOnce: true, templateButtons: but,mentions: ments ? ments : [], ...options })
    }
abot.sendButImage = async (jid, ppuser, but = [] , text = '' , footer = '', ments = [], quoted) => {
...
abot.sendMessage(jid, buttonMessage, quoted)
}
abot.sendButMessage = async (jid , desc = '' , footer = '' ,  but = [] , options = {}) =>{
const buttonMessage = {
    text: desc,
    footer: footer,
    buttons: but,
...
abot.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
      let type = await abot.getFile(path, true)
      let {
         res,
         data: file,
         filename: pathFile
      ...
      let m
      try {
         m = await abot.sendMessage(jid, message, {
            ...opt,
            ...options
         })
      }
         ...
         if (!m) m = await abot.sendMessage(jid, {
            ...message,
            [mtype]: file
         }, {
            ...opt,
    ...
    abot.sendMediaAsSticker = async (jid, path, quoted, options = {}) => {
	let { ext, mime, data } = await abot.getFile(path)
	let media = {}
let buffer
media.data = data
	media.mimetype = mime
...
return buffer
    ...
    abot.sendVideo = async (jid, path, gif = false, caption = '', quoted = '', options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await fetch(path)).buffer() : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
  ...
  let vtype
	...
	abot.sendText = (jid, text, quoted = '', options) => abot.sendMessage(jid, { text: text, ...options }, { quoted, ...options })
     ...
     * @returns 
     */
    abot.getFile = async (PATH, returnAsFilename) => {
      let res, filename
