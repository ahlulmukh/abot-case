...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./function/lib/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./function/lib/functions')
...
await
say('Abot', { font: 'console', align: 'left', gradient: ['magenta', 'red'] })
...
abot.reply = (from, content, msg) => abot.sendMessage(from, { text: content }, { quoted: msg })
...
try {
var pic = await getBuffer(await abot.profilePictureUrl(member, 'image'))
} catch {
var pic = await getBuffer(await abot.profilePictureUrl(room.id, 'image'))
}
//templete
let butwel = [{ buttonId: 'list', buttonText: { displayText: 'WELCOME' }, type: 1 }]
let butleav = [{ buttonId: 'donasi', buttonText: { displayText: 'Bye Beban👋' }, type: 1 }]
...
if (room.action == 'add') {
abot.sendMessage(room.id, { caption: txwel, location: { jpegThumbnail: await reSize(pic, 250, 250)}, buttons: butwel, footer: nyoutube, mentions: [member] })
} else if (room.action == 'remove') {
let txlev = `Selamat Tinggal Kak @${member.split('@')[0]}\n*Semoga Tenang Di Alam Sana*\n*Balik Lagi Gua Sumpahin Mandul Lu*\n_~Admin_`
abot.sendMessage(room.id, { caption: txlev, location: { jpegThumbnail: await reSize(pic, 250, 250)}, buttons: butleav, footer: nyoutube, mentions: [member] })
}
} catch (err) {
console.log(err)
}
...
if (connection === 'close') {lastDisconnect.error?.output?.statusCode !== DisconnectReason.loggedOut ? startabot() : ''}
else if(connection === 'open') {abot.sendMessage("628126915328@s.whatsapp.net", {text:`${JSON.stringify(update, undefined, 2)}`})}
console.log(update)})

abot.send5ButGif = async (jid , text = '' , footer = '', but = [], options = {}) =>{
let message = await prepareWAMessageMedia({ video: thumb, gifPlayback: true }, { upload: abot.waUploadToServer })
...
abot.send5ButImg = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
let message = await prepareWAMessageMedia({ image: img }, { upload: abot.waUploadToServer })
...
abot.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
let message = await prepareWAMessageMedia({ video: vid }, { upload: abot.waUploadToServer })
var template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
videoMessage: message.videoMessage,
...
abot.send5ButLoc = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
var template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
 "hydratedContentText": text,
...
abot.sendList = async (jid , title = '', text = '', buttext = '', footer = '', but = [], options = {}) =>{
var template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
listMessage :{
 title: title,
 description: text,
...
abot.sendTextWithMentions = async (jid, text, quoted, options = {}) => abot.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
